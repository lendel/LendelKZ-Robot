...
YTDL_REGEX = (r"^((?:https?:)?\/\/)"
              r"?((?:www|m)\.)"
              r"?((?:youtube\.com|youtu\.be|instagram\.com/tv|vk\.com))"
              r"(\/)([-a-zA-Z0-9()@:%_\+.~#?&//=]*)([\w\-]+)(\S+)?$")
...
async def start(client, message):
   if message.chat.type == 'private':
       await Jebot.send_message(
               ...
               text="""<b>Привет, я AVD Bot!
               
Я могу скачать видео или аудио с Youtube.

Нажми Справка или в любой момент отправь команду /help, чтобы узнать больше о том, как использовать меня!</b>""",   
                            reply_markup=InlineKeyboardMarkup(
                                [[
                                        InlineKeyboardButton(
                                            "Справка", callback_data="help")
                                            
                                    ],[
                                      InlineKeyboardButton(
                                            "Канал с новостями", url="https://t.me/avdrobot_news")
                                    ]]
                            ),        
...
async def about(client, message):
    if message.chat.type == 'private':   
        await Jebot.send_message(
               chat_id=message.chat.id,
               text="""<b>About AVD Bot!</b>

""",
     reply_markup=InlineKeyboardMarkup(
                                [[
                                        InlineKeyboardButton(
                                            "Назад", callback_data="help"),
                                        InlineKeyboardButton(
                                            "На главную", callback_data="start")
                                    ]]
                            ),        
            disable_web_page_preview=True,        
            parse_mode="html")
...
async def callback_query_ytdl_video(_, callback_query):
    try:
        ...
        with YoutubeDL(ydl_opts) as ydl:
            ...
            info_dict = ydl.extract_info(url, download=False)
            # download
            await callback_query.edit_message_text("**Загружаю...**")
            ydl.process_info(info_dict)
            # upload
